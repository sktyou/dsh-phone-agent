...
**OCR 读中文不会读出乱码,而是读出形近字**:土→士、招→拍、都→郡、单→半、莴→莒。

**`findtext` / `wait` 会自动折叠这些** —— 搜「星耀天都店」能命中识别成
「星耀天郡店」的文本。**遇到"明明在屏幕上却找不到"时,先怀疑这个已经处理了。**

折叠表来自实测(不是通用混淆表 —— 猜的条目会让无关字符串相撞,而**错误匹配比漏匹配更危险**,因为漏匹配看得见)。

**但要注意**:折叠只影响**匹配**,不改变**返回的文本** —— `match.text` 仍是识别出的原文。

## OCR 三档质量/速度旋钮

| 参数 | 默认 | 作用 | 代价 |
|---|---|---|---|
| `scale` | 1.0(`sweep` 1.5) | 识别前放大 | 像素×scale² |
| `enhance` | `sweep` 开 | 低质量时补一遍对比度增强并合并 | +380ms |
| `refine` | 关 | 对低置信行**逐行裁剪重识别** | 最多 +0.5s/行 |
...
|---|---|---|---|
| 无 | 634ms | — | — |
| enhance | 1017ms | 0.637 | 33 |
| enhance + refine | 3161ms | 0.649 | 30 |

**`refine` 的收益很小**(+1.9% 置信度),**默认关闭**。它只值得用在"文字必须准"
的场景(采集商品名),不值得用在"只需要定位"的场景。

**`refine` 能修什么、不能修什么**(实测):

```
"¥5.9预估到手)"  → "¥5.9预估到手"     ✅ 修掉多余符号
"6.1折|一口价"   → "6.1折 一口价"     ✅ 修掉竖线
"【高配】单人自选5件套" → 相同(置信 0.64→0.75)  ✅ 提置信
"超拍手品质精选"  → "超招手品质精选"   ❌ 仍然错
```

**裁剪修的是"整图给错了参数",不是"识别器读不出来"。**

## OCR 引擎:两个,按场景选

| 引擎 | 单屏 | 置信度 | 确定性 | 适合 |
|---|---|---|---|---|
| `mlkit`(默认) | 1.5s | 中位 0.66 | ❌ **同图两次结果不同** | 快速定位、拉丁文本 |
| `ppocr` | 2.7s | **中位 0.99** | ✅ 同图同结果 | **采集文字**、中文小字彩色文本 |

**实测同一菜单**:ML Kit 标题有乱码("FIY\BA")、置信度 0.4-0.8;
PP-OCR 逐字正确、置信度中位 0.993。

```json
{"cmd":"ocr","engine":"ppocr","region":[230,440,1080,2200]}
{"cmd":"sweep","ocrEngine":"ppocr"}     // sweep 默认就是 ppocr
```

**PP-OCR 输出已归一化**:全角 `￥`(U+FFE5)→ 半角 `¥`。否则 `￥9.9` 和 `¥9.9`
比不相等,每个下游都要自己写一遍替换。

## 动态页面:用 `sequence` 的 `locateTap`

**轮播、自动滚动、懒加载列表会在"定位"和"点击"之间移走目标。**

**实测:单次 `locate` 耗时 829ms** —— 两次调用之间的空窗足够让卡片滑走。

```json
{"cmd":"sequence","steps":[
  {"action":"locateTap","target":"选规格"}]}
→ {"ok":true,"found":true,"tapped":true,
   "strategy":"textContains","confidence":0.85,"center":[972,822]}
```

**定位和点击在同一次调用内完成**,中间只有微秒。返回里带 `strategy` / `confidence`
/ `center`,便于判断这次是不是真的点对了。

## 页面自己会变时:先确认状态

**App 无法控制目标应用的行为**,只能帮你检测。常见几种:

| 现象 | 检测方法 |
|---|---|
| **定位/地址漂移**(外卖类 App 反复横跳) | `findtext "请选择地址"`;或用 `wait` 等地址栏文字出现 |
| **同名门店**("星耀天都店" vs "星耀天郡店") | 用**两个词同时匹配**("蔡大胖"+"星耀"),别只用"星耀" |
| **门店未营业** | 搜索结果里会有"接受预订"字样,`findtext` 可判断 |
| **弹窗/蒙层挡住内容区** | `tap` 的 `safety.code` 会给 `obscured` |
